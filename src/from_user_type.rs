    ...
    let generated = quote! {
        impl scylla::cql_to_rust::FromCqlVal<scylla::frame::response::result::CqlValue> for #struct_name {
            ...
            -> Result<Self, scylla::cql_to_rust::FromCqlValError> {
                ...
                // For compatibility reasons we allow these unused fields (in case the struct is not in sync with the cql schema)
                // Warning, this could cause the loss of the data if the UDT is overwritten.
                // if let Some(e) = fields_iter.next() {
                //     eprintln!("{:?}", e);
